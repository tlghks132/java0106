...
public class Lotto {
	...
	public static void main(String[] args) {
		...
		Scanner sc = new Scanner(System.in);  //입력 받은때는 System.in
		...
		for(int i=0; i<6; i=i+1) {
			...
			int su = sc.nextInt();
			//1부터 45까지 숫자만 저장 하고 그 외의 경우는 다시 입력하도록 하기
			if(su < 1 || su >45) {
			...
			//데이터 중복 검사 (#0번째 데이터 부터 현재 잎려갈 데이터 앞까지 중복 검사)
			if(i > 0 ) {
				boolean flag = false;
				//첫번째 데이터부터 현재 입력 할 데이터 앞까지 비교
